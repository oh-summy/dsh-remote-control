...
case "$OS" in Darwin) os=darwin;; Linux) os=linux;; *) echo "[dsh-web] ✗ 不支持的系统: ${OS}（Windows 欢迎提 PR）"; exit 1;; esac
case "$ARCH" in x86_64) arch=amd64;; aarch64|arm64) arch=arm64;; *) echo "[dsh-web] ✗ 不支持的架构: $ARCH"; exit 1;; esac
echo "[dsh-web] 平台: $os-$arch"
...
if [ -x "$BIN_DIR/cloudflared" ] || command -v cloudflared >/dev/null 2>&1; then
  ...
  echo "[dsh-web] 下载 cloudflared $CFV ($os-$arch) ..."
  if [ "$os" = darwin ]; then
...
if [ -x "$BIN_DIR/caddy" ] || command -v caddy >/dev/null 2>&1; then
  ...
  VER="${TAG#v}"
  echo "[dsh-web] 下载 caddy $TAG ($os-$arch) ..."
  ...
  [ -n "${RC_CADDY_VERSION:-}" ] && echo "[dsh-web] ⚠ darwin 走官方构建 API，仅提供最新版，忽略 RC_CADDY_VERSION"
  echo "[dsh-web] 下载 caddy ($os-$arch, 官方构建 API, 裸二进制 ~35MB, 不支持断点) ..."
  i=0
  while [ $i -lt 5 ]; do
    i=$((i+1))
    curl -fSL -m 900 --speed-limit 1024 --speed-time 60 \
      -o "$BIN_DIR/caddy" "https://caddyserver.com/api/download?os=$os&arch=$arch" && break
    echo "[dsh-web] 第 $i 次下载未完成，重试（全新下载）..."
    sleep 2
  done
  [ -x "$BIN_DIR/caddy" ] || { echo "[dsh-web] ✗ caddy 下载失败"; exit 1; }
  chmod +x "$BIN_DIR/caddy"
fi
...
case ":$PATH:" in
  *":$LINK_DIR:"*) echo "[dsh-web] 命令已安装: dsh-web（${LINK_DIR}）" ;;
  *) echo "[dsh-web] 命令已安装: $LINK_DIR/dsh-web（注意：$LINK_DIR 不在 PATH）" ;;
esac
...
echo "[dsh-web]   1) 编辑 $RC_HOME/rc.env，填入 RC_FEISHU_WEBHOOK（飞书群机器人地址）"
echo "[dsh-web]   2) 运行 $REPO_DIR/bin/up.sh 启动远程入口"
echo "[dsh-web]   3) 运行 $REPO_DIR/bin/status.sh 查看状态"